...
#
# Each example wraps its body in `ActiveRecord::Base.transaction(requires_new: true)`
# so any PG::InFailedSqlTransaction abort from an inner uniqueness violation is
# contained by a savepoint, keeping the outer spec transaction valid for cleanup.
RSpec.describe "Order number race condition (savepoint)", type: :model do
