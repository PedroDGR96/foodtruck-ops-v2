...
RSpec.describe PaymentPolicy do
  ...
  it "lets cashiers view payments with a gateway reference" do
    payment = order.payments.build(method: "pix", amount: 10.00, gateway_reference: "test_ref")
    expect(policy_for(cashier).show?(payment)).to be(true)
  end
end
